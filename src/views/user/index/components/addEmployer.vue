<template>
  ...
  >
    <!-- 表单 -->
  <el-form ref="addEmptyForm"  label-width="120px" :model="formData" :rules="rules">
   <el-form-item label="人员名称：" prop="userName">
        <el-input style="width:80%" placeholder="请输入姓名" v-model="formData.userName"  maxlength="5"
  show-word-limit />
    </el-form-item>
    <el-form-item label="角色:" prop="roleId">
        <el-select style="width:80%" placeholder="请选择角色" v-model="formData.roleId" >
            <el-option v-for="item in rolesList" :key="item.roleId" :label="item.roleName" :value="item.roleId" />
        </el-select>
      </el-form-item>
    <el-form-item label="联系电话:" prop="mobile">
        <el-input style="width:80%" placeholder="请输入联系电话" v-model="formData.mobile"  maxlength="11"
  show-word-limit />
    </el-form-item>
    <el-form-item label="负责区域:" prop="regionId">
        <el-select style="width:80%" placeholder="请选择区域" v-model="formData.regionId" >
            <el-option v-for="item in regionList" :key="item.id" :label="item.name" :value="item.id"  />
        </el-select>
      </el-form-item>
      <el-form-item label="头像:" prop="image">
         <!-- 放置上传图片 -->
          <UploadImg v-model="formData.img" ref="employeesHeader" @onSuccess="headerImgSuccess" />
      </el-form-item>
  ...
  <div slot="footer" class="dialog-footer">
    <el-button type="warning" @click="addFn" v-loading="loading">确 定</el-button>
  ...
  data() {
    ...
    methods: {
      ...
      async addFn() {
        ...
        this.loading= true
         //根据有无id判断是修改角色还是新增接口
         this.formData.id ? await updateRole(this.formData)
